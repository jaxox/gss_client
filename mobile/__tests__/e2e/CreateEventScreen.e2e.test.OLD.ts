...
describe('CreateEventWizard E2E Tests', () => {
  beforeAll(async () => {
    ...
    // Disable synchronization to avoid React Native Paper animation issues
    await device.disableSynchronization();
  });

  beforeEach(async () => {
    await device.reloadReactNative();
    // Wait longer for app to settle after reload when synchronization is disabled
    ...
    // Wait for wizard step 1 to load (text is "BASIC INFORMATION" in all caps)
  ...
  afterAll(async () => {
    // Re-enable synchronization after tests
  ...
  describe('Step 1 - Basic Information', () => {
    ...
    it('should display sport options', async () => {
      // Scroll down to see all sports
      await element(by.id('step1-scroll-view')).scroll(200, 'down');
    ...
    it('should show character counters', async () => {
      ...
      await detoxExpect(element(by.text('0/50 characters'))).toBeVisible();
      await detoxExpect(element(by.text('0/1000 characters'))).toBeVisible();
    });
    ...
    it('should show validation errors for empty fields', async () => {
      ...
      // Verify next button is visible (testing disabled state is unreliable in Detox)
      const nextButton = element(by.id('next-button'));
      await detoxExpect(nextButton).toBeVisible();

      // Try tapping - should not navigate (button is disabled)
    ...
    it('should show validation error for short description', async () => {
      // Fill valid title
      await element(by.id('event-title-input')).typeText('Valid Title');
      ...
      // Should still be on Step 1 (description too short)
      await detoxExpect(element(by.text('STEP 1 OF 4'))).toBeVisible();
    });
    ...
    it('should have a working Cancel button', async () => {
      ...
      // Scroll to bottom where cancel button is
      await element(by.id('step1-scroll-view')).scrollTo('bottom');
      await new Promise(resolve => setTimeout(resolve, 300));
  ...
  describe('Complete Wizard Flow - All 4 Steps', () => {
    it('should successfully complete all wizard steps and publish event', async () => {
      ...
      // Enter location
      const locationInput = element(by.id('location-input-pressable'));
      ...
      await detoxExpect(element(by.id('time-input-pressable'))).toBeVisible();
      await detoxExpect(
        element(by.id('end-time-input-pressable')),
      ).toBeVisible();
    ...
    it('should preserve data when navigating back and forth', async () => {
      ...
      await element(by.text('BASIC INFORMATION')).tap();
      await new Promise(resolve => setTimeout(resolve, 500));
      await element(by.id('sport-card-table tennis')).tap();
      await element(by.id('next-button')).tap();
      ...
      await waitFor(element(by.text('STEP 2 OF 4')))
        .toBeVisible()
        .withTimeout(5000);
      // Tap location to open modal
      await element(by.id('location-input-pressable')).tap();
      await new Promise(resolve => setTimeout(resolve, 500));
      // Type in search field
      await element(by.id('location-search-input')).typeText(testLocation);
      await new Promise(resolve => setTimeout(resolve, 300));
      // Save location
    ...
    it('should reach Step 3 by filling date/time pickers', async () => {
      ...
      await waitFor(element(by.text('STEP 2 OF 4')))
        .toBeVisible()
        .withTimeout(5000);
      // Step 2: Fill location and time
      await waitFor(element(by.text('STEP 2 OF 4')))
        .toBeVisible()
        .withTimeout(5000);

      // Fill location via modal
      await element(by.id('location-input-pressable')).tap();
      await new Promise(resolve => setTimeout(resolve, 500));
      ...
      // Fill start time using E2E mode text input
      await element(by.id('time-input-pressable')).replaceText('14:30');
      await element(by.text('LOCATION & TIME')).tap();
      await new Promise(resolve => setTimeout(resolve, 300));

      // Fill end time using E2E mode text input
      ...
      await waitFor(element(by.id('time-input-pressable')))
        .toBeVisible(75)
      ...
      await new Promise(resolve => setTimeout(resolve, 300));
      await element(by.id('time-input-pressable')).replaceText('14:00');
      await new Promise(resolve => setTimeout(resolve, 500));

      // Fill end time
      await waitFor(element(by.id('end-time-input-pressable')))
        .toBeVisible(75)
        .whileElement(by.id('step2-scroll-view'))
        .scroll(100, 'down');
      ...
      await new Promise(resolve => setTimeout(resolve, 300));
      await element(by.id('end-time-input-pressable')).replaceText('16:00');
      await new Promise(resolve => setTimeout(resolve, 500));
